...
$(window).on("load", async function () {
  ...
  showMessage(
    "message-4",
    "<strong>INFO: </strong>We have now updated our api! It is now much faster and does not error. Click <a href='https://github.com/Lixqa/DiscordPomelo#API'>here</a> to read the docs."
  );
  $(".check").on("click", async function (e) {
    ...
    if (!res.error) {
      ...
      if (res.data.check.status == 0) {
        showMessage(
          "message-3",
          `🚨 Error(s):<br>${res.data.check.errors
            .map((e) => e.message)
            .join("<br><hr>")}`
        );
        ...
        showMessage(
          "message-3",
          `🚨 Looks like all <strong>${res.data.check.attempt}</strong> attempts to check your username failed. If this error still occurs in 1 hour, contact us on discord.`
        );
        ...
        showMessage(
          "message-3",
          "🚨 Something went wrong. Check console and report on discord"
        );
      }
    } else {
      if (res.code == "RATE_LIMITED")
        return showMessage(
          "message-3",
          `⌛ Slow down for ${Math.floor(res.data.retryAfter / 1000)} seconds!`
        );
      if (res.code == "TEMPORARILY_DISABLED")
        return showMessage("message-3", "Temporarily disabled for maintenance");
      showMessage(
        "message-3",
        `🚨 ${res.message} | If this still happends in a hour, report on: <a href='https://discord.gg/8n7kfX6S4h'>discord.gg/8n7kfX6S4h</a>`
      );
    }
  });
